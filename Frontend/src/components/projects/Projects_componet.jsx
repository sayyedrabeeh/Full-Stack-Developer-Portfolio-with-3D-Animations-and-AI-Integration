...
import { Heart, MessageCircle, Share2Icon, Github, ExternalLink, ChevronLeft, ChevronRight, Calendar, Send, X } from "lucide-react"
...
export default function Project_Component({ Project_type }) {
    ...
    const toggle_like = async (id) => {
        const token = localStorage.getItem('access')
        const res = await fetch(`${baseURL}/api/accounts/projects/${id}/likes`, {
            method: 'POST',
    ...
    const addComment = async () => {
        ...
        await fetch(`${baseURL}/api/accounts/projects/${currentProjectId}/comments`, {
            method: 'POST',
            headers: {
    ...
    const openComments = async (projectId) => {
        ...
        const token = localStorage.getItem('access')
        const res = await fetch(`${baseURL}/api/accounts/projects/${projectId}/get_comments/`, {
            headers: { "Authorization": `Bearer ${token}` }
        })
        const data = await res.json()
            ...
            <main className="max-w-2xl mx-auto px-4 py-4 space-y-6 pb-20">
                {project.length === 0 ? (
                    ...
                    project.map((p) => {
                        ...
                        return (
                            <article key={p.id}
                                className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden border border-gray-200 dark:border-gray-700 transition-all duration-300">
                                ...
                                <div className="flex items-center gap-5 p-3 border-b border-gray-100 dark:border-gray-700">
                                    <button onClick={() => toggle_like(p.id)}
                                        className={`flex items-center gap-1.5 transition-all ${p.userLiked ? 'text-red-500' : 'text-gray-700 dark:text-gray-300'} hover:scale-110`}>
                                        <Heart className={`w-6 h-6 ${p.userLiked ? 'fill-current' : ''}`} />
                                ...
                                <div className="px-4 pb-3 space-y-2">
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {p.tech_stack?.split(',').map((t) => t.trim()).filter(Boolean).map((t, i) => (
                                            <span key={i} className="text-xs font-medium text-blue-600 dark:text-blue-400">
                                                #{t.replace(/\s+/g, "")}
                                            </span>
                                        ))}
                                    </div>
