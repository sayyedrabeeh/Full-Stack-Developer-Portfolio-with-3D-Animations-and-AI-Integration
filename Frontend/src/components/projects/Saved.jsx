...
const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMore && !loading) {
          fetchSavedProjects(project.length);
        }
      },
      { threshold: 0.5 }
    );
  ...
  }, [hasMore, loading, fetchSavedProjects, project.length]);
  ...
  const toggle_like = async (id) => {
    ...
    setProject((prev) =>
      prev.map((p) =>
        p.id === id ? { ...p, likes: data.likes, userLiked: data.liked } : p
      )
    );
  };
  ...
  const handleBookmark = async (id) => {
    try {
      const res = await api.post(
        `/api/accounts/projects/${id}/bookmark/`,
        {},
        {
          headers: {
      ...
      setProject((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, is_bookmarked: status === "added" } : p
        )
      );
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      <header className="sticky top-0 z-10 bg-gray-800 border-b border-gray-700">
        <div className="max-w-2xl mx-auto flex items-center justify-center px-4 py-3">
          <div className="mb flex flex-col items-center gap-1">
            ...
            <p className="text-sm text-gray-400">Loved feed of my work</p>
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 space-y-6 pb-20">
        {project.length === 0 ? (
          <p className="text-center text-gray-400 py-12">No Saved Projects Yet.</p>
        ) : (
          project.map((p) => {
            ...
            return (
              ...
              >
                          ...
                          {p.images.map((_, i) => (
                            <div
                              ...
                              className={`h-1 rounded-full transition-all ${
                                i === imgIdx ? "bg-white w-7" : "bg-white/50 w-2"
                              }`}
                            />
                          ))}
                        </div>
                ...
                <div className="flex items-center gap-5 p-3">
                  <button
                    onClick={() => toggle_like(p.id)}
                    className={`flex items-center gap-1.5 transition-all ${
                      p.userLiked ? "text-red-500" : "text-gray-300"
                    } hover:scale-110`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 transition-all hover:scale-110 ${
                      isCommentsOpen ? "text-blue-400" : "text-gray-300"
                    }`}
                  >
                  ...
                  >
                    {p.is_bookmarked ? <BookmarkCheck className="w-6 h-6" /> : <Bookmark className="w-6 h-6" />}
                  </button>
                </div>
                <div
                  className={`overflow-hidden transition-all duration-300 ease-in-out ${
                    isCommentsOpen ? "max-h-[600px] opacity-100" : "max-h-0 opacity-0"
                  }`}
                >
                    ...
                    <div className="max-h-64 overflow-y-auto space-y-3 mb-4 pr-2 scrollbar-thin scrollbar-thumb-gray-600">
                      {comments.length > 0 ? (
                        comments.map((c) => (
                          <div
                            key={c.id}
                            className="bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-700 flex items-start gap-2"
                          >
                ...
                <div className="px-4 pb-3 space-y-2">
                  <p className="text-sm text-gray-100 leading-relaxed">{p.description}</p>
